...
- `/jobs/[slug]`: 메타 정보, 안전하게 렌더링한 공고 Markdown, 관련 문서 존재 배지
- 모바일과 데스크톱에서 사용할 수 있는 반응형 레이아웃
- 데이터 없음, 잘못된 설정, 일부 문서 파싱 실패에 대한 안내 상태

과한 그래프나 관리 기능보다는 공고를 찾고 내용을 읽는 흐름을 우선한다.
...
초기 MVP 이후 사용자 요청으로 잡코리아, 원티드, 사람인의 공개 채용공고와 Schema.org `JobPosting`을 제공하는 회사 채용 페이지의 제한 수집을 추가했다. robots 정책, 요청 간격, 사이트별 상한을 지키며 결과는 Git에서 제외된 로컬 캐시에만 저장한다. 자동 수집 공고는 실제 지원 공고와 별도 유형으로 관리한다.

## 완료 조건

- [ ] `.env.example`을 `.env`로 복사하고 데이터 경로만 설정할 수 있다.
- [ ] `docker compose up --build` 후 `http://localhost:3000`에서 앱이 열린다.
- [ ] 실제 `career-workbench`가 read-only로 마운트되고 공고 목록과 상세가 표시된다.
- [ ] 실제 자료가 없어도 저장소의 가상 fixture를 지정해 기능을 확인할 수 있다.
- [ ] 검색, 상태 필터, 정렬, 상세 이동이 동작한다.
