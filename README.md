...
기본 설정은 잡코리아, 원티드, 사람인의 공개 카테고리·키워드 검색 목록을 최대 5페이지까지 확인합니다. 사이트·검색 직무별 후보를 최대 120개까지 탐색하고, 프로필에 맞는 공고가 30개 찰 때까지 상세 공고를 보충 확인합니다. 로그인, 지원서, 이력서, 회원 정보 API에는 접근하지 않습니다. 각 사이트의 `robots.txt`를 확인하고 요청 사이에 최소 600ms 간격을 둡니다. 같은 회사·직무의 중복 공고는 화면에서 하나로 합칩니다.
