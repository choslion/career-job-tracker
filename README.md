...
- 지원 현황 요약, 브라우저에서 즉시 반응하는 검색·상태 필터
- 공고 본문은 저장하지 않고 원문 링크로 연결
...
파서와 앱은 원본 디렉터리에 파일을 생성하거나 수정하지 않습니다. 관련 지원 문서는 정해진 파일의 존재 여부만 확인하고 본문을 목록 데이터로 전달하지 않습니다. 수집기도 공고 본문을 저장하지 않고 회사·포지션·마감일·근무지·태그 같은 메타 정보만 캐시에 남깁니다.

## 화면

- `/`: 상태별 지원 수와 가까운 마감일 요약
- `/jobs`: 채용공고 검색, 직무·지역·상태 필터, 정렬

두 화면 모두 서버는 공고 목록을 한 번만 내려보내고, 이후 검색과 필터는 브라우저에서 처리합니다. 공고 상세 화면은 두지 않습니다. 공고 제목이나 "원문에서 상세 보기"를 누르면 원본 채용공고가 새 탭에서 열립니다.
